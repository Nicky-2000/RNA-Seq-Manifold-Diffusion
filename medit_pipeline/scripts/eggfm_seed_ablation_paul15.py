from __future__ import annotations
import argparse
import copy
from pathlib import Path
from typing import Any, Dict, List

import random
import numpy as np
import scanpy as sc
import yaml
from sklearn.cluster import KMeans
from sklearn.metrics import adjusted_rand_score
import subprocess
import torch

from EGGFM.eggfm import run_eggfm_dimred
from EGGFM.prep import prep_for_manifolds


def build_argparser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser()
    ap.add_argument("--params", required=True, help="configs/params.yml")
    ap.add_argument(
        "--out_txt",
        default="out/eggfm_seed_ablation_paul15.txt",
        help="Where to write seed ablation results",
    )
    ap.add_argument(
        "--gcs_path",
        default=None,
        help="Optional GCS path (e.g. gs://bucket/dir/) to copy the txt file to",
    )
    return ap


def set_seed(seed: int) -> None:
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(seed)


def compute_ari(
    qc_ad: sc.AnnData,
    emb_key: str,
    label_key: str,
    n_dims: int,
) -> float:
    """Compute ARI from an embedding in qc_ad.obsm[emb_key]."""
    if emb_key not in qc_ad.obsm:
        raise ValueError(f"Embedding {emb_key!r} not found in qc_ad.obsm")

    X = qc_ad.obsm[emb_key]
    if X.ndim != 2:
        raise ValueError(f"Embedding {emb_key!r} must be 2D, got shape={X.shape}")

    if label_key not in qc_ad.obs:
        raise ValueError(f"Label key {label_key!r} not found in qc_ad.obs")

    labels = qc_ad.obs[label_key].to_numpy()
    unique_labels = np.unique(labels)
    if unique_labels.size < 2:
        raise ValueError(
            f"Need at least 2 unique labels for ARI; got {unique_labels.size}"
        )

    n_clusters = unique_labels.size
    k_eff = min(n_dims, X.shape[1])
    km = KMeans(n_clusters=n_clusters, n_init=10, random_state=0)
    km.fit(X[:, :k_eff])
    ari = adjusted_rand_score(labels, km.labels_)
    return float(ari)


def main() -> None:
    print("[seed_ablation] starting", flush=True)
    args = build_argparser().parse_args()
    print("[seed_ablation] parsed args", flush=True)

    params: Dict[str, Any] = yaml.safe_load(Path(args.params).read_text())
    print("[seed_ablation] loaded params", flush=True)

    spec = params.get("spec", {})
    label_key = spec.get("ari_label_key", None)
    if label_key is None:
        raise ValueError("spec.ari_label_key must be set in params.yml")
    ari_n_dims = int(spec.get("ari_n_dims", spec.get("n_pcs", 10)))

    out_txt_path = Path(args.out_txt)
    out_txt_path.parent.mkdir(parents=True, exist_ok=True)

    # ---- best-config hyperparams (from your earlier ablation) ----
    BEST_HIDDEN_DIMS = [256]
    BEST_SIGMA = 0.05
    BEST_METRIC_GAMMA = params.get("eggfm_diffmap", {}).get("metric_gamma", 0.2)
    BEST_METRIC_LAMBDA = 4.0
    BEST_EPS_TRUNC = "no"
    BEST_T = 2.0
    BEST_N_NEIGHBORS = 15
    BEST_NUM_EPOCHS = params.get("eggfm_train", {}).get("num_epochs", 50)

    # You can change these if you want to test hvg=1000 or batch=1024 instead
    BEST_HVG_N = 2000
    BEST_BATCH_SIZE = 2048

    # Seeds to test
    seed_grid: List[int] = [0, 1, 2, 3, 4]

    # ---- write header ----
    if not out_txt_path.exists():
        with out_txt_path.open("w") as f:
            f.write(
                "dataset\tseed\thvg_n_top_genes\tbatch_size\t"
                "hidden_dims\tsigma\tmetric_gamma\tmetric_lambda\t"
                "eps_trunc\tt\tn_neighbors\tnum_epochs\t"
                "ari_diffmap_eggfm\tari_diffmap_eggfm_x2\n"
            )

    dataset_name_base = "paul15"
    ari_eggfm_list: List[float] = []
    ari_eggfm_x2_list: List[float] = []

    combo_idx = 0

    for seed in seed_grid:
        combo_idx += 1
        print(
            f"[seed_ablation] === run {combo_idx} / {len(seed_grid)} with seed={seed} ===",
            flush=True,
        )
        set_seed(seed)

        # ---- load raw paul15 and prep with fixed HVG ----
        print(
            f"[seed_ablation] loading paul15 and prepping for HVG={BEST_HVG_N}...",
            flush=True,
        )
        ad = sc.datasets.paul15()
        # Adjust this call if your prep_for_manifolds signature is different:
        qc_base = prep_for_manifolds(ad, hvg_n_top_genes=BEST_HVG_N)
        dataset_name = f"{dataset_name_base}_hvg{BEST_HVG_N}"

        print(
            f"[seed_ablation] qc_base shape={qc_base.shape} for HVG={BEST_HVG_N}",
            flush=True,
        )

        # Build cfg with best hyperparams + fixed batch_size
        cfg = copy.deepcopy(params)
        cfg.setdefault("eggfm_model", {})
        cfg.setdefault("eggfm_train", {})
        cfg.setdefault("eggfm_diffmap", {})

        cfg["eggfm_model"]["hidden_dims"] = BEST_HIDDEN_DIMS
        cfg["eggfm_train"]["sigma"] = float(BEST_SIGMA)
        cfg["eggfm_train"]["batch_size"] = int(BEST_BATCH_SIZE)
        cfg["eggfm_train"]["num_epochs"] = int(BEST_NUM_EPOCHS)

        cfg["eggfm_diffmap"]["metric_gamma"] = float(BEST_METRIC_GAMMA)
        cfg["eggfm_diffmap"]["metric_lambda"] = float(BEST_METRIC_LAMBDA)
        cfg["eggfm_diffmap"]["eps_trunc"] = str(BEST_EPS_TRUNC)
        cfg["eggfm_diffmap"]["t"] = float(BEST_T)
        cfg["eggfm_diffmap"]["n_neighbors"] = int(BEST_N_NEIGHBORS)

        # Work on a copy of qc_base
        qc_run = qc_base.copy()

        # ---- 1) Run EGGFM + pure EGGFM Diffmap ----
        qc_run, _ = run_eggfm_dimred(qc_run, cfg)
        # qc_run.obsm["X_eggfm"] is pure EGGFM DM under this config.

        # ---- 2) Double diffusion on top of X_eggfm ----
        n_pcs = int(spec.get("n_pcs", 10))
        sc.pp.neighbors(
            qc_run,
            n_neighbors=BEST_N_NEIGHBORS,
            use_rep="X_eggfm",
        )
        sc.tl.diffmap(qc_run, n_comps=n_pcs)
        qc_run.obsm["X_diff_eggfm_x2"] = qc_run.obsm["X_diffmap"][:, :n_pcs]

        # ---- 3) Compute ARIs ----
        try:
            ari_eggfm = compute_ari(qc_run, "X_eggfm", label_key, ari_n_dims)
        except Exception as e:
            print(
                f"[seed_ablation] ARI failed for X_eggfm (seed={seed}): {e}",
                flush=True,
            )
            ari_eggfm = float("nan")

        try:
            ari_eggfm_x2 = compute_ari(qc_run, "X_diff_eggfm_x2", label_key, ari_n_dims)
        except Exception as e:
            print(
                f"[seed_ablation] ARI failed for X_diff_eggfm_x2 (seed={seed}): {e}",
                flush=True,
            )
            ari_eggfm_x2 = float("nan")

        ari_eggfm_list.append(ari_eggfm)
        ari_eggfm_x2_list.append(ari_eggfm_x2)

        print(
            f"[seed_ablation] seed={seed} ARIs: "
            f"eggfm={ari_eggfm:.4f}, eggfm_x2={ari_eggfm_x2:.4f}",
            flush=True,
        )

        # ---- 4) Append per-seed line to txt file ----
        with out_txt_path.open("a") as f:
            f.write(
                f"{dataset_name}\t{seed}\t{BEST_HVG_N}\t{BEST_BATCH_SIZE}\t"
                f"{BEST_HIDDEN_DIMS}\t{BEST_SIGMA}\t{BEST_METRIC_GAMMA}\t{BEST_METRIC_LAMBDA}\t"
                f"{BEST_EPS_TRUNC}\t{BEST_T}\t{BEST_N_NEIGHBORS}\t{BEST_NUM_EPOCHS}\t"
                f"{ari_eggfm:.6f}\t{ari_eggfm_x2:.6f}\n"
            )

    # ---- summary stats ----
    mean_eggfm = float(np.nanmean(ari_eggfm_list))
    std_eggfm = float(np.nanstd(ari_eggfm_list))
    mean_eggfm_x2 = float(np.nanmean(ari_eggfm_x2_list))
    std_eggfm_x2 = float(np.nanstd(ari_eggfm_x2_list))

    print(
        "[seed_ablation] SUMMARY over seeds " f"{seed_grid}:",
        flush=True,
    )
    print(
        f"  EGGFM DM:      mean ARI = {mean_eggfm:.4f}, std = {std_eggfm:.4f}",
        flush=True,
    )
    print(
        f"  EGGFM DM x2:   mean ARI = {mean_eggfm_x2:.4f}, std = {std_eggfm_x2:.4f}",
        flush=True,
    )

    # Append summary as a pseudo-row
    with out_txt_path.open("a") as f:
        f.write(
            f"{dataset_name_base}_summary\tmean({seed_grid})\t{BEST_HVG_N}\t{BEST_BATCH_SIZE}\t"
            f"{BEST_HIDDEN_DIMS}\t{BEST_SIGMA}\t{BEST_METRIC_GAMMA}\t{BEST_METRIC_LAMBDA}\t"
            f"{BEST_EPS_TRUNC}\t{BEST_T}\t{BEST_N_NEIGHBORS}\t{BEST_NUM_EPOCHS}\t"
            f"{mean_eggfm:.6f}±{std_eggfm:.6f}\t{mean_eggfm_x2:.6f}±{std_eggfm_x2:.6f}\n"
        )

    print(f"[seed_ablation] done, results at {out_txt_path}", flush=True)

    # ---- Optional: copy to GCS ----
    if args.gcs_path:
        print(
            f"[seed_ablation] copying {out_txt_path} to {args.gcs_path} via gsutil cp",
            flush=True,
        )
        try:
            subprocess.run(
                ["gsutil", "cp", str(out_txt_path), args.gcs_path],
                check=False,
            )
        except Exception as e:
            print(f"[seed_ablation] gsutil cp failed: {e}", flush=True)


if __name__ == "__main__":
    main()
